<template>
  <h2>Find your perfect dance shoes!</h2>
...
export default {
  ...
  methods: {
    /* fetchProducts() hämtar produktdatan från den lokala JSON-filen shoes.json
      som sedan tilldelas till this.products och this.filteredCategory så alla
      produkter visas initialt.*/
    ...
    filterProducts(filter) {
      ...
      if (filter === "All") {
        ...
        this.filteredCategory = this.products.filter(
          (product) => product.name.toLowerCase() === filter.toLowerCase()
        );
      }
    },
  },
